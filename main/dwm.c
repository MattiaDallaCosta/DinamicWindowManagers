...
static int drawstatusbar(Monitor *m, int bh, int vpad, int hpad, char *text);
...
int drawstatusbar(Monitor *m, int bh, int vpad, int hpad, char *stext) {
  int ret, i, w, x, len;
  short isCode = 0;
  char *text;
  char *p;
  char buf1[8], buf2[8];
  memcpy(buf1, colors[SchemeStatus][ColFg], 8);
  memcpy(buf2, colors[SchemeStatus][ColBg], 8);
    ...
    if (text[i] == '^' && !isCode) {
      ...
      while (text[++i] != '^') {
        if (text[i] == 'c') {
          ...
          memcpy(buf1, (char *)text + i + 1, 7);
          buf1[7] = '\0';
          const char *newscheme[] = {buf1, buf2, "#000000"};
          drw_setscheme(
              drw, drw_scm_create(drw, newscheme, alphas[SchemeStatus], 3));
          i += 7;
        } else if (text[i] == 'b') {
          memcpy(buf2, (char *)text + i + 1, 7);
          buf2[7] = '\0';
          const char *newscheme[] = {buf1, buf2, "#000000"};
          drw_setscheme(
              drw, drw_scm_create(drw, newscheme, alphas[SchemeStatus], 3));
          ...
          drw_rect(drw, rx + x, ry + hpad, rw, rh, 1, 0);
        } else if (text[i] == 'f') {
          x += atoi(text + ++i);
        }
      }
...
void drawbar(Monitor *m) {
  ...
  tw = m->ww - drawstatusbar(m, bh, vp, sp, stext);
...
void setdirs(const Arg *arg) {
  int *dirs = (int *)arg->v, i, n;
  Area *areas = selmon->pertag->areas[selmon->pertag->curtag];

  for (i = 0; i < 3; i++) {
    n = (int[]){4, 2, 2}[i];
    areas[i].dir = ISINC(dirs[i]) ? MOD((int)areas[i].dir + GETINC(dirs[i]), n)
                                  : TRUNC(dirs[i], 0, n - 1);
  }
  arrange(selmon);
}

void setfacts(const Arg *arg) {
  ...
  for (i = 0; i < 3; i++)
    areas[i].fact = TRUNC(
        ISINC(facts[i]) ? areas[i].fact + GETINC(facts[i]) : facts[i], 0.1, 10);
  arrange(selmon);
}
...
void tile(Monitor *m) {
  ...
  if (ga->dir == DirHor || ga->dir == DirRotHor)
    ...
    ma->y = 0 + m->ogappx, ma->fx = ma->x + ms - 2 * m->ogappx,
    ma->fy = m->wh - m->ogappx,
    sa->x = ga->dir == DirHor ? ms + g - m->ogappx : 0 + m->ogappx,
    sa->y = 0 + m->ogappx, sa->fx = sa->x + ss, sa->fy = m->wh - m->ogappx;
  else
    ms = f * (m->wh - g), ss = m->wh - ms - g, ma->x = 0 + m->ogappx,
    ma->y = ga->dir == DirVer ? 0 + m->ogappx : ss + g + m->ogappx,
    ma->fx = m->ww - m->ogappx, ma->fy = ma->y + ms - 2 * m->ogappx,
    sa->x = 0 + m->ogappx,
    sa->y = ga->dir == DirVer ? ms + g - m->ogappx : 0 + m->ogappx,
    sa->fx = m->ww - m->ogappx, sa->fy = sa->y + ss;
  /* tile clients */
